 ...
 * @param {Buffer|String} metadata Buffer with metadata information
 * @returns {Object}
 * @private
 */
function _parseMetadata(metadata) {
    metadata = Buffer.isBuffer(metadata) ? metadata.toString('utf8') : metadata || '';
    metadata = metadata.replace(/\0*$/, '').split(';');
 ...
 * @private
 */
StreamReader.prototype._transform = function (chunk, encoding, done) {
    _onData(this, chunk, this.push.bind(this), done);
};
