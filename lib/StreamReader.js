 ...
 * @param {Buffer} chunk One chunk of data
 * @param {Function} output Function for pass through chunk
 * @param {Function} done Callback when processing is done
 * @returns {Function}
 * @private
 */
var _processData = function _processData(stream, chunk, output, done) {
    stream._readerBytesLeft -= chunk.length;
    ...
    if (stream._readerBytesLeft === 0) {
        ...
        cb.apply(stream, [chunk, output]);
    }

    return done;
};

var _onData = _trampoline(function _onData(stream, chunk, output, done) {
    if (stream._readerBytesLeft <= 0) {
        return done(new Error("Transform stream is not parsing data anymore"));
    }

    if (chunk.length <= stream._readerBytesLeft) {
        return function () {
            return _processData(stream, chunk, output, done);
        };
    } else {
        return function () {
            var b = chunk.slice(0, stream._readerBytesLeft);
            return _processData(stream, b, output, function (err) {
                if (err) return done(err);
                if (chunk.length > b.length) {
                    return function () {
                        return _onData(stream, chunk.slice(b.length), output, done);
                    };
                }
            });
        };
